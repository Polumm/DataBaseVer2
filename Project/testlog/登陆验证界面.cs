...
namespace testlog
{
    public partial class 登陆验证界面 : Form
    {
        ...
        private bool login()
        {
           ...
           if(comboBox1.Text == "学生")
            {
                string sql = "select 用户名, 密码 from 学生密码表 where 用户名 ='"+ textBox1.Text + "' and 密码 = '" + textBox2.Text + "'";
                Door dr = new Door();
            ...
            if(comboBox1.Text == "教师")
            {
                string sql = "select 用户名, 密码 from 教师密码表 where 用户名 ='" + textBox1.Text + "' and 密码 = '" + textBox2.Text + "'";
                Door dr = new Door();
                IDataReader Student_Door = dr.Reader(sql);//Reader是Door封装的读入方法
                if (Student_Door.Read())//.Read()是IDataReader自带的方法，若成功读到信息，返回1
                {
            ...
            {   //务必小心，sql是字符串，如果其中的引号是中文字符，编译器不会报错！
                string sql = "select 用户名, 密码 from 管理员密码表 where 用户名 ='"+ textBox1.Text +"' and 密码 = '"+ textBox2.Text + "'";
                Door dr = new Door();
